...
def parse_flags(tokens: list) -> dict:
    """Parse option flags: vp, ring, fame1/fame2/fame3. Defaults all off."""
    options = {"vp": False, "ring": False, "fame": 0}
    for token in tokens:
...
def breakdown(price: int, vp: bool = False, ring: bool = False, fame: int = 0) -> dict:
...
def run(arguments: str) -> str:
    ...
    Usage: /bdotax <price> [vp] [ring] [fame1|fame2|fame3]
    ...
    if not tokens:
        raise BdoTaxError(
            "usage: /bdotax <price> [vp] [ring] [fame1|fame2|fame3]\n"
            "examples: /bdotax 100m  ·  /bdotax 1.5b vp  ·  /bdotax 5000000 vp ring fame3"
        )

    price = parse_price(tokens[0])
    options = parse_flags(tokens[1:])

    if not any([options["vp"], options["ring"], options["fame"]]):
    ...
    return format_breakdown(
        breakdown(price, vp=options["vp"], ring=options["ring"], fame=options["fame"])
    )
