...
- Comma semantics differ by command, matching each community's convention: the calculator reads `2,5` as a decimal comma (= 2.5; inside parentheses commas separate function arguments, e.g. `pow(2,10)`), while `/bdotax` and `/hourly` read `1,000,000` as thousands separators.
